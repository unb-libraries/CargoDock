...
rsync -a ${RSYNC_FLAGS} "${DRUPAL_BUILD_TMPROOT}/core/profiles/${DRUPAL_BASE_PROFILE}/config" "${DRUPAL_BUILD_TMPROOT}/profiles/${DRUPAL_SITE_ID}/"
...
if [[ -d "$ADDITIONAL_CONFIG_DIR" ]]; then
  rsync -a ${RSYNC_FLAGS} ${ADDITIONAL_CONFIG_DIR} ${DRUPAL_BUILD_TMPROOT}/profiles/${DRUPAL_SITE_ID}/
fi

# Move local profile from repo to build root, overwriting.
cd ${DRUPAL_BUILD_TMPROOT}
rsync -a ${RSYNC_FLAGS} --remove-source-files ${TMP_DRUPAL_BUILD_DIR}/${DRUPAL_SITE_ID} ${DRUPAL_BUILD_TMPROOT}/profiles/
